...
const addEmployees = asyncHandler(async(req, res) =>{
    ...
    const{ firstName, lastName, qualification, yearOfExperience, department, email, phoneNumber} = req.body;
    console.log(firstName);
    // Check all the required fields are not empty
    if([firstName, lastName, yearOfExperience, department, phoneNumber].some((field)=>field?.trim() === "")){
    ...
    if(!employees){
        throw new apiError(500, "Something went wrong while registering an employee")
    }
